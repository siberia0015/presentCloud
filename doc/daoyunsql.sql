...
CREATE TABLE `t_class`  (
  ...
  `joinable` tinyint UNSIGNED NULL DEFAULT 1 COMMENT '是否可加入',
...
CREATE TABLE `t_dict_info`  (
  ...
  `sequence` int UNSIGNED NULL DEFAULT 1 COMMENT '排序',
...
CREATE TABLE `t_pagebutton`  (
  `pageButton_id` int NOT NULL AUTO_INCREMENT,
  `button_id` int NULL DEFAULT NULL,
...
CREATE TABLE `t_sign`  (
  ...
  `class_id` int NOT NULL COMMENT '班课id',
  `sign_time` datetime NOT NULL COMMENT '签到时间',
  `score` int UNSIGNED NULL DEFAULT 2 COMMENT '经验值（默认2）',
  `start_sign_id` int NOT NULL COMMENT '签到事件号',
...
CREATE TABLE `t_start_sign`  (
  `id` int NOT NULL AUTO_INCREMENT COMMENT '签到事件号',
  `user_id` bigint NOT NULL COMMENT '用户id（签到发起人）',
  `class_id` int NOT NULL COMMENT '班课id',
  `start_time` datetime NOT NULL COMMENT '签到发起时间',
  `type` int NOT NULL COMMENT '签到类型',
  `time_limit` int UNSIGNED NULL DEFAULT 3 COMMENT '签到时限（该属性是限时签到才有）',
  `end_time` datetime NULL DEFAULT NULL COMMENT '签到结束时间',
  `over` tinyint(1) NULL DEFAULT NULL COMMENT '签到是否结束（bool）',
  `score` int UNSIGNED NULL DEFAULT 2 COMMENT '经验值（默认2）',
  `distance` int UNSIGNED NULL DEFAULT 10 COMMENT '距离',
...
CREATE TABLE `t_user_class`  (
  ...
  `identity` tinyint UNSIGNED NULL DEFAULT 0 COMMENT '创建或加入',
  `score` int UNSIGNED NULL DEFAULT 0 COMMENT '学生经验值',
  `Creator` varchar(20) CHARACTER SET utf8 COLLATE utf8_general_ci NULL DEFAULT NULL,
  `Modifier` varchar(20) CHARACTER SET utf8 COLLATE utf8_general_ci NULL DEFAULT NULL,
  `ModificationDate` datetime NULL DEFAULT NULL,
  `CreationDate` datetime NULL DEFAULT NULL,
